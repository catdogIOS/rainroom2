...
public class CityFunction : CavasData
{
    ...
    void endg()
    {
        if (PlayerPrefs.GetInt("cityending", 0) == 0)
        {
            ...
            if (GMC.GetComponent<EndingBox>().shopNum == 5 || GMC.GetComponent<EndingBox>().shopNum == 6)
            {
                GMC.GetComponent<EndingBox>().side_obj.SetActive(false);
            }
            else
            {
                GMC.GetComponent<EndingBox>().side_obj.SetActive(true);
            }
        }
    }
    ...
    IEnumerator outTime()
    {
        ...
        while (a == 0)
        {
            ...
            else
            {
                ...
                string lastTime = PlayerPrefs.GetString("outlasttimecity", dateTime.ToString());

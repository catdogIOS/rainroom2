...
public class SleepTime : MonoBehaviour
{
    ...
    void SleepTimeFlow()
    {
        ...
        System.DateTime d = System.DateTime.UtcNow.AddHours(-7);
        string lastTime = PlayerPrefs.GetString("sleepLastTime", d.ToString());
        ...
        {
            lastTime = System.DateTime.UtcNow.AddHours(-7).ToString();
        }
        lastDateTime = System.DateTime.Parse(lastTime);
    ...
    IEnumerator sleepImagechange()
    {
        ...
        while (aa == 0)
        {
            ...
            if (m == 2)
            {
                m = PlayerPrefs.GetInt("bedmaxlv", 0);
            }

            if (bed == 0)
            {
                sleep_obj[0].GetComponent<Image>().sprite = sleep1_spr[n];
                sleepMax_obj.GetComponent<Image>().sprite = sleepMax1_spr[m];
                bed++;
            }
            else
            {
                sleep_obj[0].GetComponent<Image>().sprite = sleep2_spr[n];
                sleepMax_obj.GetComponent<Image>().sprite = sleepMax2_spr[m];
                bed = 0;
            }
